...
/**
 * @brief Unified logging macros mimicking standard ESP_LOG definitions.
 */
#define LOG_ERROR(tag, format, ...) do { \
    ESP_LOGE(tag, format, ##__VA_ARGS__); \
    log_manager_write_history("[ERR] ", format, ##__VA_ARGS__); \
} while(0)

#define LOG_WARN(tag, format, ...) do { \
    ESP_LOGW(tag, format, ##__VA_ARGS__); \
    log_manager_write_history("[WRN] ", format, ##__VA_ARGS__); \
} while(0)

#define LOG_INFO(tag, format, ...) do { \
    ESP_LOGI(tag, format, ##__VA_ARGS__); \
    log_manager_write_history("[INF] ", format, ##__VA_ARGS__); \
} while(0)
